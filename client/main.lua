...
RegisterNetEvent('ox_doorlock:setState', function(id, state, source, data)
	...
	if Config.Notify and source == cache.serverId then
		...
		elseif Config.NotifyType == 'sandbox' or Config.NotifyType == 'pulsar' then
			if state == 0 then
				exports['pulsar-hud']:Notification("success", locale('unlocked_door'))
			else
				exports['pulsar-hud']:Notification("error", locale('locked_door'))
			end
		end
	end
	...
	while true do
		...
		elseif showUI then
			if Config.DrawTextUIType == 'ox' then
				lib.hideTextUI()
			elseif Config.DrawTextUIType == 'sandbox' or Config.DrawTextUIType == 'pulsar' then
				exports['pulsar-hud']:ActionHide("door_action")
			end
			showUI = nil
		end
